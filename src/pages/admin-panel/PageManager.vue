<template>
  <div
    class="admin-content-box admin-content-box-grow"
    @click="openComponent"
    :class="{ 'admin-content-box-open': isOpen }"
  >
    <h2 @click="closeComponent" class="cursorPointer adminHeader">
      Insert / remove / swap pages / thumbnail
    </h2>
    ...
    <span class="admin-content-box-inner" v-if="isOpen">
      ...
      <div class="horizontalFlex margin-top-8 flex-wrap">
        ...
        <select v-model="comic" @change="comicChanged">
          <option v-for="comic in comicOptions" :key="comic.id" :value="comic">
            {{ comic.name }}
          </option>
        </select>
        <a
          :href="`https://yiffer.xyz/${comic.name}`"
          v-if="comic"
      ...
      <div v-if="comic" class="horizontalFlex" style="margin-top: 8px">
        <p class="admin-mini-header" style="margin-right: 8px">
          Start page viewing range:
        </p>
      ...
      <div v-if="comic" class="horizontalFlex flex-wrap">
        ...
        >
          <p>Page {{ i }}</p>
          <a
            :href="`${config.comicsBaseUrl}/${comic.name}/${formattedPageNumber(
              i
            )}.jpg?${generateRandomQueryString()}`"
            target="_blank"
          >
            <img
              :src="`${config.comicsBaseUrl}/${
                comic.name
              }/${formattedPageNumber(i)}.jpg?${generateRandomQueryString()}`"
      ...
      <span v-if="comic" class="admin-content-box-inner">
        ...
        <button
          ...
          v-if="swapPage1 && swapPage2 && swapPage1 != swapPage2"
        >
          Swap
        </button>
        ...
        <p>
          Insert a page in between two other pages. <u>Don't</u> use this for
          adding a page to the end of a comic. For that, go to
          <i>Add pages to comic</i>
        </p>
        <div class="horizontalFlex">
          ...
          <p v-if="insertPageAfterNumber">
            , and before page {{ insertPageAfterNumber + 1 }}
          </p>
        </div>

        <span
          v-if="insertPageAfterNumber"
          style="display: flex; align-items: center; flex-direction: column"
        >
          <form
            enctype="multipart/form-data"
            novalidate
            class="margin-top-16 no-margin-bot"
          ...
          <p v-if="imageToInsert">
            Selected file: <span class="courier">{{ imageToInsert.name }}</span>
          </p>
        ...
        <p>
          Delete an existing page, in case of duplicate pages (or for some other
          reason)
        </p>
        <div class="horizontalFlex">
        ...
        >
          <div class="pretty-input-upload">
            <input
              type="file"
              @change="processThumbnailUploadChange"
              accept="image/png,image/jpeg"
              class="input-file"
            />
            <p>Select file</p>
        ...
        <p v-if="thumbnailImageFile">
          Selected file:
          <span class="courier">{{ thumbnailImageFile.name }}</span>
        </p>

        <button
          @click="replaceThumbnail"
          v-if="thumbnailImageFile && !thumbnailImageWrongFormat"
          class="y-button"
        >
          Insert {{ thumbnailImageFile.name }}
        </button>
      ...
      <menu-up-icon
        ...
        class="mdi-arrow close-component-arrow"
      />
    </span>
...
export default {
  ...
  methods: {
    async swapPages() {
      ...
      let response = await comicApi.swapComicPages(
        this.comic.name,
        this.comic.id,
        this.swapPage1,
        this.swapPage2
      ...
      if (response.success) {
        this.setResponseMessage(
          "success",
          `Successfully swapped pages ${this.swapPage1} and ${this.swapPage2}`
        );
        ...
        this.setResponseMessage(
          "error",
          "Error inserting page: " + response.message
        );
      }
    },
    ...
    async insertPage() {
      ...
      try {
        await comicApi.insertComicPage(
          this.comic.name,
          this.comic.id,
          this.imageToInsert,
        ...
        this.setResponseMessage(
          "success",
          "Successfully inserted new page " +
            (Number(this.insertPageAfterNumber) + 1)
        );
        ...
        this.setResponseMessage(
          "error",
          err.response?.data || "Unknown server error"
        );
        return;
      }
    },
    ...
    async deletePage() {
      ...
      if (response.success) {
        this.setResponseMessage(
          "success",
          `Successfully deleted page ${this.deletePageNumber}`
        );
        this.$store.dispatch("updateOneComicInList", this.comic);
        ...
        this.setResponseMessage(
          "error",
          "Error deleting page: " + response.message
        );
      }
    },
    ...
    async replaceThumbnail() {
      ...
      let response = await comicApi.replaceThumbnailImage(
        this.comic.name,
        this.comic.id,
      ...
      if (response.success) {
        ...
        this.setResponseMessage(
          "error",
          "Error replacing thumbnail: " + response.message
        );
      }
    },

    updateUploadProgress(progressEvent) {
      this.responseMessage = `Uploading... ${Math.round(
    ...
    openComponent() {
      if (!this.isOpen) {
        setTimeout(() => (this.isOpen = true), 15);
      }
    },

    closeComponent() {
      setTimeout(() => (this.isOpen = false), 15);
    },
  },
