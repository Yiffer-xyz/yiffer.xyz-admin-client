    ...
    <span class="admin-content-box-inner" v-if="isOpen">
      ...
      <div v-if="comic" class="horizontalFlex flex-wrap">
        <div v-for="i in pagesToShow" :key="i" class="verticalFlex" style="margin: 4px;">
          <p>Page {{i}}</p>
          <a :href="`${config.comicsBaseUrl}/${comic.name}/${formattedPageNumber(i)}.jpg?${generateRandomQueryString()}`" target="_blank">
            <img :src="`${config.comicsBaseUrl}/${comic.name}/${formattedPageNumber(i)}.jpg?${generateRandomQueryString()}`" 
